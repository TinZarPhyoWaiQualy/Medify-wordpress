<?php
/*template name: blog2column */
get_header();
?>

  <main>
      <section class="blog2column-section">
        <div class="blog2column">
          <div class="inner-blog2column">
            <h2 class="blog2column-h2 blog2column-header">
              Grid 2 Columns + Sidebar
            </h2>
            <div class="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="/">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">
                  Grid 2 Columns + Sidebar
                </li>
              </ol>
            </div>
          </div>
        </div>
      </section>

      <section class="blog2column-slider-sidebar-wrapper">
        <div class="blog2column-siderbar-wrapper">
          <div class="slider-card-wrapper">
            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">TIPS</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/common/specialist.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  How to Choose the Specialist in Massage
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">BEAUTY</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/common/nursingcare.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  Hospital Nursing Care: Some Observations
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">TIPS</span>
                <div class="wlslider">
                  <div>
                    <img src="<?php bloginfo('template_directory'); ?>/assets/img/common/boostrix.jpg" alt="Slide 1" />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  Boostrix for Vacination of Adults Against Diphteria
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">HEALTH</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/chid-insomnia.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  Causes And Cures For Childhood Insomnia
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">HEALTH</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/Physician-1.jpg"
                      alt="Slide 1"
                    />
                  </div>
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/Physician-2.jpg"
                      alt="Slide 2"
                    />
                  </div>
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/Physician-3.jpg"
                      alt="Slide 3"
                    />
                  </div>
                </div>
                <button class="custom-prev">
                  <img src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/prev-arrow.png" />
                </button>
                <button class="custom-next">
                  <img src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/next-arrow.png" />
                </button>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  10 Tips For Being A Terrific Traveling Physician
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">VACINATION</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/gooddoctor.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">10 Steps To Finding A Good Doctor</h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">BEAUTY</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/healthinsurance.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  The Joys Of Health Insurance Bureaucracy
                </h3>
              </div>
            </div>

            <div class="slider-card">
              <div class="slider-wrapper">
                <span class="badge">VACINATION</span>
                <div class="wlslider">
                  <div>
                    <img
                      src="<?php bloginfo('template_directory'); ?>/assets/img/blog2column/designfeature.jpg"
                      alt="Slide 1"
                    />
                  </div>
                </div>
              </div>
              <div class="post-info">
                <span class="post-date">AUGUST 5, 2019</span>
                <h3 class="post-title">
                  The Ten Worst Hospital Design Features
                </h3>
              </div>
            </div>
          </div>

          <div class="blog-single-sidebar">
            <sidebar class="sidebar-sticky">
              <img
                src="<?php bloginfo('template_directory'); ?>/assets/img/common/marrylee.png"
                class="marrylee-img"
              />
              <h5 class="marry-name">Marry Lee</h5>
              <p class="author">
                Hi! I`m an author of this blog. Read our post - be in trend!
              </p>
              <div class="online-app">
                <div class="app-wrapper">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/twitter.png"
                    class="app base-img"
                    alt="base"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/twitter-float.png"
                    class="app float-img"
                    alt="float"
                  />
                </div>
                <div class="app-wrapper">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/facebook.png"
                    class="app base-img"
                    alt="base"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/facebook-float.png"
                    class="app float-img"
                    alt="float"
                  />
                </div>
                <div class="app-wrapper">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/linkedin.png"
                    class="app base-img"
                    alt="base"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/linkedin-float.png"
                    class="app float-img"
                    alt="float"
                  />
                </div>
                <div class="app-wrapper">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/instagram.png"
                    class="app base-img"
                    alt="base"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/instragram-float.png"
                    class="app float-img"
                    alt="float"
                  />
                </div>
              </div>

              <div class="categories-wrapper">
                <h6 class="categories">CATEGORIES</h6>
                <div class="category-list">
                  <div class="category-item">
                    <span class="category">Beauty</span>
                    <span class="dash-line"></span>
                    <span class="page-number">4</span>
                  </div>
                  <div class="category-item">
                    <span class="category">Health</span>
                    <span class="dash-line"></span>
                    <span class="page-number">5</span>
                  </div>
                  <div class="category-item">
                    <span class="category">Tips</span>
                    <span class="dash-line"></span>
                    <span class="page-number">6</span>
                  </div>
                  <div class="category-item">
                    <span class="category">Vacination</span>
                    <span class="dash-line"></span>
                    <span class="page-number">4</span>
                  </div>
                </div>
              </div>

              <div class="gallery-wrapper">
                <h5 class="gallery">GALLERY</h5>
                <div class="gallery-gp">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_01.jpg"
                    alt="gallery_01"
                    class="gallery-img"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_02.jpg"
                    alt="gallery_02"
                    class="gallery-img"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_03.jpg"
                    alt="gallery_03"
                    class="gallery-img"
                  />
                </div>
                <div class="gallery-gp">
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_04.jpg"
                    alt="gallery_04"
                    class="gallery-img"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_05.jpg"
                    alt="gallery_05"
                    class="gallery-img"
                  />
                  <img
                    src="<?php bloginfo('template_directory'); ?>/assets/img/common/gallery_06.jpg"
                    alt="gallery_06"
                    class="gallery-img"
                  />
                </div>
                <div class="widget-banner-wrapper">
                  <div class="widget-banner">
                    <div class="widget">
                      <h2 class="emergemcy-service">
                        Call for an Emergency Service!
                      </h2>
                      <span class="emergency-number">+8 (123) 456 789 12</span>
                      <div class="emergencycall-wrapper">
                        <a href="#" class="emergency-call"
                          ><span>Call Now</span></a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </sidebar>
          </div>
        </div>
      </section>
    </main>
<?php
get_footer();